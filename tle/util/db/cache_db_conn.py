...
class CacheDbConn:
    ...
    def fetch_problems(self):
        query = 'SELECT contest_id, problemset_name, [index], name, type, points, rating, tags FROM problem'
        res = self.conn.execute(query).fetchall()
    ...
    def save_rating_changes(self, changes):
        ...
        query = 'INSERT OR REPLACE INTO rating_change (contest_id, handle, rank, rating_update_time, old_rating, new_rating) VALUES (?, ?, ?, ?, ?, ?)'
    ...
    def get_users_with_more_than_n_contests(self, time_cutoff, n):
        query = 'SELECT handle, COUNT(*) AS num_contests FROM rating_change GROUP BY handle HAVING num_contests >= ? AND MAX(rating_update_time) >= ?'
    ...
    def get_all_rating_changes(self):
        query = 'SELECT contest_id, name, handle, rank, rating_update_time, old_rating, new_rating FROM rating_change r LEFT JOIN contest c ON r.contest_id = c.id ORDER BY rating_update_time'
        res = self.conn.execute(query)
        return (cf.RatingChange._make(change) for change in res)

    def get_rating_changes_for_contest(self, contest_id):
        query = 'SELECT contest_id, name, handle, rank, rating_update_time, old_rating, new_rating FROM rating_change r LEFT JOIN contest c ON r.contest_id = c.id WHERE r.contest_id = ?'
    ...
    def get_rating_changes_for_handle(self, handle):
        query = 'SELECT contest_id, name, handle, rank, rating_update_time, old_rating, new_rating FROM rating_change r LEFT JOIN contest c ON r.contest_id = c.id WHERE r.handle = ?'
    ...
    def fetch_problems2(self):
        query = 'SELECT contest_id, problemset_name, [index], name, type, points, rating, tags FROM problem2 '
        res = self.conn.execute(query).fetchall()
        return list(map(self._unsquish_tags, res))
    ...
    def fetch_problemset(self, contest_id):
        query = 'SELECT contest_id, problemset_name, [index], name, type, points, rating, tags FROM problem2 WHERE contest_id = ?'
        res = self.conn.execute(query, (contest_id,)).fetchall()
        return list(map(self._unsquish_tags, res))
