...
class CacheDbConn:
    ...
    def create_tables(self):
        ...
        self.conn.execute('CREATE TABLE IF NOT EXISTS contest ('
                          'id             INTEGER NOT NULL,'
                          'name           TEXT,'
                          'start_time     INTEGER,'
                          'duration       INTEGER,'
        ...
        self.conn.execute('CREATE TABLE IF NOT EXISTS problem ('
                          'contest_id       INTEGER,'
        ...
        self.conn.execute('CREATE TABLE IF NOT EXISTS rating_change ('
        ...
        self.conn.execute('CREATE TABLE IF NOT EXISTS problem2 ('
                          'contest_id       INTEGER,'
                          'problemset_name  TEXT,'
                          '[index]          TEXT,'
                          'name             TEXT NOT NULL,'
    ...
    def save_rating_changes(self, changes):
        change_tuples = [
            (change.contestId, change.handle, change.rank, change.ratingUpdateTimeSeconds,
             change.oldRating, change.newRating) for change in changes
        ]
    ...
    def cache_problemset(self, problemset):
        query = (
            ...
            '(contest_id, problemset_name, [index], name, type, points, rating, tags) '
            'VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
        rc = self.conn.executemany(query, list(map(self._squish_tags,
                                                   problemset))).rowcount
        self.conn.commit()
        return rc
