...
class ProblemsetCache:
    ...
    async def run(self):
        if self.cache_master.conn.problemset_empty():
            self.logger.warning(
                'Problemset cache on disk is empty. This must be populated manually before use.'
            )
        self._update_task.start()
    ...
    async def _update_task(self, _):
        async with self.update_lock:
            ...
            self.logger.info(
                f'{len(new_problems)} new problems saved and {len(updated_problems)} saved problems updated.'
            )
...
class RatingChangesCache:
    ...
    async def run(self):
        ...
        if not self.handle_rating_cache:
            self.logger.warning(
                'Rating changes cache on disk is empty. This must be populated manually before use.'
            )
        self._update_task.start()
