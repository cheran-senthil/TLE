...
class ProblemCache:
    ...
    @tasks.task_spec(name='ProblemCacheUpdate', waiter=tasks.Waiter.fixed_delay(_RELOAD_INTERVAL))
    async def _update_task(self, _):
        async with self.reload_lock:
    ...
    async def _update(self, problems):
        ...
        contest_map = {problem.contestId: self.cache_master.contest_cache.contest_by_id.get(problem.contestId) for problem in problems}
...
class ProblemsetCache:
    ...
    async def run(self):
        if self.cache_master.conn.problemset_empty():
            self.logger.warning('Problemset cache on disk is empty. This must be populated manually before use.')
        self._update_task.start()
    ...
    @tasks.task_spec(name='ProblemsetCacheUpdate', waiter=tasks.Waiter.fixed_delay(_RELOAD_DELAY))
    async def _update_task(self, _):
        async with self.update_lock:
            contests = self.cache_master.contest_cache.contests_by_phase['FINISHED']
            ...
            self.logger.info(f'{len(new_problems)} new problems saved and {len(updated_problems)} saved problems updated.')
    ...
    async def _fetch_problemsets(self, contests, *, force_fetch=False):
        ...
        for contest_id, rated_problem_idx in contests_to_refetch:
            updated_problems += [prob for prob in await self._fetch_for_contest(contest_id) if prob.rating is not None and prob.index not in rated_problem_idx]

        return new_problems, updated_problems

    async def _fetch_for_contest(self, contest_id):
        try:
            _, problemset, _ = await cf.contest.standings(contest_id=contest_id, from_=1, count=1)
...
class RatingChangesCache:
    ...
    async def run(self):
        ...
        if not self.handle_rating_cache:
            self.logger.warning('Rating changes cache on disk is empty. This must be populated manually before use.')
        self._update_task.start()
    ...
    def is_newly_finished_without_rating_changes(self, contest):
        now = time.time()
        return contest.phase == 'FINISHED' and now - contest.end_time < self._RATED_DELAY and not self.has_rating_changes_saved(contest.id)
    ...
    async def _update_task(self, _):
        ...
        to_monitor = [contest for contest in self.cache_master.contest_cache.contests_by_phase['FINISHED'] if self.is_newly_finished_without_rating_changes(contest) and not _is_blacklisted(contest)]
    ...
    async def _monitor_task(self, _):
        self.monitored_contests = [contest for contest in self.monitored_contests if self.is_newly_finished_without_rating_changes(contest) and not _is_blacklisted(contest)]
        ...
        for contest, changes in contest_changes_pairs:
            cf_common.event_sys.dispatch(events.RatingChangesUpdate, contest=contest, rating_changes=changes)
    ...
    def get_current_rating(self, handle, default_if_absent=False):
        return self.handle_rating_cache.get(handle, cf.DEFAULT_RATING if default_if_absent else None)
...
class RanklistCache:
    ...
    async def _update_task(self, _):
        ...
        finished_contests = [contest for contest in contests_by_phase['FINISHED'] if not _is_blacklisted(contest) and rating_cache.is_newly_finished_without_rating_changes(contest)]
    ...
    async def _monitor_task(self, _):
        ...
        self.monitored_contests = [contest for contest in self.monitored_contests if not _is_blacklisted(contest) and (contest.phase != 'FINISHED' or cache.is_newly_finished_without_rating_changes(contest))]
    ...
    async def _get_contest_details(contest_id, show_unofficial):
        contest, problems, standings = await cf.contest.standings(contest_id=contest_id, show_unofficial=show_unofficial)

        # Exclude PRACTICE and MANAGER
        standings = [row for row in standings if row.party.participantType in ('CONTESTANT', 'OUT_OF_COMPETITION', 'VIRTUAL')]
    ...
    async def _get_ranklist_with_fetched_changes(self, contest_id, show_unofficial):
        ...
        if is_rated:
            ...
            delta_by_handle = {change.handle: change.newRating - change.oldRating for change in changes}
    ...
    async def _get_ranklist_with_predicted_changes(self, contest_id, show_unofficial):
        ...
        if cf_common.is_nonstandard_contest(contest) or has_teams:
            ...
            current_rating = {row.party.members[0].handle: current_rating.get(row.party.members[0].handle, 1500) for row in standings_official}
    ...
    async def generate_vc_ranklist(self, contest_id, handle_to_member_id):
        ...
        contest, problems, standings = await cf.contest.standings(contest_id=contest_id, show_unofficial=True)
        # Exclude PRACTICE, MANAGER and OUR_OF_COMPETITION
        standings = [row for row in standings if row.party.participantType == 'CONTESTANT' or row.party.members[0].handle in handles]
        ...
        current_official_rating = {rating_change.handle: rating_change.oldRating for rating_change in rating_changes}

        # TODO: assert that none of the given handles are in the official standings.
        handles = [row.party.members[0].handle for row in standings if row.party.members[0].handle in handles and row.party.participantType == 'VIRTUAL']
        current_vc_rating = {handle: cf_common.user_db.get_vc_rating(handle_to_member_id.get(handle)) for handle in handles}
        ranklist = Ranklist(contest, problems, standings, now, is_rated=True)
        delta_by_handle = {}
...
class CacheSystem:
    ...
    async def getUsersEffectiveRating(*, activeOnly=None):
        ...
        users_effective_rating_dict = {user.handle: user.effective_rating for user in ratedList}
