...
def is_nonstandard_problem(problem):
    return is_nonstandard_contest(cache2.contest_cache.get_contest(problem.contestId)) or problem.matches_all_tags(['*special'])
...
class HandleIsVjudgeError(ResolveHandleError):
    HANDLES = ('vjudge1 vjudge2 vjudge3 vjudge4 vjudge5 luogu_bot1 luogu_bot2 luogu_bot3 luogu_bot4 luogu_bot5').split()
...
async def resolve_handles(ctx, converter, handles, *, mincnt=1, maxcnt=5, default_to_all_server=False):
    ...
    if '+server' in handles:
        ...
        guild_handles = {handle for discord_id, handle in user_db.get_handles_for_guild(ctx.guild.id)}
...
class SubFilter:
    ...
    def filter_subs(self, submissions):
        ...
        for submission in submissions:
            ...
            if self.rated:
                ...
                problem_ok = not contest or contest.id >= cf.GYM_ID_THRESHOLD or not is_nonstandard_problem(problem)
                rating_ok = True
    ...
    def filter_rating_changes(self, rating_changes):
        rating_changes = [change for change in rating_changes if self.dlo <= change.ratingUpdateTimeSeconds < self.dhi]
