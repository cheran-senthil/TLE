...
class Round(commands.Cog):
    ...
    async def _check_if_all_members_ready(self, ctx, members):
        embed = discord.Embed(
            description=f'{" ".join(x.mention for x in members)} react on the message with ✅ within 30 seconds to join the round. {"Since you are the only participant, this will be a practice round and there will be no rating changes" if len(members) == 1 else ""}',
    ...
    def _check_if_any_member_is_already_in_round(self, ctx, members):
        ...
        if busy_members:
            busy_members_str = ', '.join([
                ctx.guild.get_member(int(member.id)).mention for member in busy_members
            ])
    ...
    @round.command(
        name='challenge',
        brief='Challenge multiple users to a round',
        usage='[@user1 @user2...]',
    )
    async def challenge(self, ctx, *members: discord.Member):
        ...
        problem_cnt = await self._get_time_response(
            self.bot,
            ctx,
        ...
        duration = await self._get_time_response(
            self.bot,
            ctx,
        ...
        ratings = await self._get_seq_response(
            self.bot,
            ctx,
        ...
        points = await self._get_seq_response(
            self.bot,
            ctx,
    ...
    async def _update_round(self, round_info):
        ...
        for i in range(len(problems)):
            ...
            times = [
                await self._get_solve_time(
                    recent_subs[index],
                    int(problems[i].split('/')[0]),
                    problems[i].split('/')[1],
                )
        ...
        if over:
            ...
            eloChanges = self._calculateRatingChanges([
                [
                    ...
                    user.rank,
                    cf_common.user_db.get_duel_rating(user.id, guild.id),
                ]
                for user in ranklist
            ])
        ...
        def _make_pages(data, title):
            ...
            for chunk in chunks:
                ...
                for round in chunk:
                    ...
                    msg += ' vs '.join([
                        f'[{cf_common.user_db.get_handle(user.id, round.guild)}](https://codeforces.com/profile/{cf_common.user_db.get_handle(user.id, round.guild)}) `Rank {user.rank}` `{user.points} Points`'
                        for user in ranklist
                    ])
                    msg += f'\n**Problem ratings:** {round.rating}'
                    msg += f'\n**Score distribution** {round.points}'
                    timestr = cf_common.pretty_time_format(
                        ((round.time + 60 * round.duration) - int(time.time())),
                        shorten=True,
                        always_seconds=True,
                    )
                    msg += f'\n**Time left:** {timestr}\n\n'
        ...
        def _make_pages(data, title):
            ...
            for chunk in chunks:
                ...
                for round in chunk:
                    ...
                    msg += f'\n**Score distribution** {round.points}'
                    timestr = cf_common.pretty_time_format(
                        min(60 * round.duration, round.end_time - round.time),
                        shorten=True,
                        always_seconds=True,
                    )
                    msg += f'\n**Duration:** {timestr}\n\n'
