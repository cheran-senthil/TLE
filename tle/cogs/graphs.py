...
def _get_extremes(contest, problemset, submissions):
    def in_contest(sub):
        return (sub.author.participantType == 'CONTESTANT'
                or (cf_common.is_rated_for_onsite_contest(contest)
...
def _plot_extreme(handle, rating, packed_contest_subs_problemset, solved,
                  unsolved, legend):
    extremes = [(dt.datetime.fromtimestamp(contest.end_time),
    ...
    if legend:
        plt.legend(title=f'{handle}: {rating}',
...
class Graphs(commands.Cog):
    ...
    async def extreme(self, ctx, *args: str):
        ...
        rating = max(
            ratingchanges,
            key=lambda change: change.ratingUpdateTimeSeconds).newRating
        _plot_extreme(handle, rating, packed_contest_subs_problemset, solved,
                      unsolved, legend)
    ...
    @plot.command(
        ...
        usage=
        '[handles] [+practice] [+contest] [+virtual] [+outof] [+team] [+tag..] [r>=rating] [r<=rating] [d>=[[dd]mm]yyyy] [d<[[dd]mm]yyyy] [c+marker..] [i+index..]'
    )
    ...
    @plot.command(
        ...
        usage=
        '[handles] [+practice] [+contest] [+virtual] [+outof] [+team] [+tag..] [r>=rating] [r<=rating] [d>=[[dd]mm]yyyy] [d<[[dd]mm]yyyy] [phase_days=] [c+marker..] [i+index..]'
    )
    ...
    @plot.command(
        ...
        aliases=['chilli'],
        usage=
        '[handle] [+practice] [+contest] [+virtual] [+outof] [+team] [+tag..] [r>=rating] [r<=rating] [d>=[[dd]mm]yyyy] [d<[[dd]mm]yyyy] [b=10] [s=3] [c+marker..] [i+index..] [+nolegend]'
    )
    ...
    @plot.command(
        brief=
        'Show Codeforces rating distribution, possibly for a subset of countries',
        usage='[+normal] [+log] [+all] [contest_cutoff=5] [countries..]')
    ...
    async def cfdistrib(self, ctx, *args: str):
        ...
        (isnormal, islog,
         isall), args = cf_common.filter_flags(args,
                                               ['+normal', '+log', '+all'])
        ...
        if countries:
            ...
            handles = [
                user.handle for user in infos if user.country in countries
            ]
    ...
    async def country(self, ctx, *countries):
        ...
        if not countries:
            ...
            data = [
                [user.country, user.rating] for _, user in users
            ...
            discord_file = gc.get_current_figure_as_file()
            embed = discord_common.cf_color_embed(
                title='Rating distribution of server members by '
                'country')
    ...
    async def visualrank(self, ctx, contest_id: int, *args: str):
        ...
        if in_server:
            ...
            rating_changes = [
                rating_change for rating_change in rating_changes
        ...
        if not rating_changes:
            raise GraphCogError(
                f'No rating changes for contest `{contest_id}`')

        users_to_mark = {}
        for rating_change in rating_changes:
            user_delta = rating_change.newRating - rating_change.oldRating
            if rating_change.handle in handles:
                users_to_mark[rating_change.handle] = (rating_change.rank,
                                                       user_delta)
        ...
        for rating_change in rating_changes:
            ...
            if (xmin - xmargin <= rating_change.rank <= xmax + xmargin
                ...
                color.append(
                    cf.rating2rank(rating_change.oldRating).color_graph)
