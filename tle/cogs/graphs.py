...
def _plot_extreme(handle, rating, packed_contest_subs_problemset, solved, unsolved,
    ...
    if fullsolves:
        scatter_outline(*zip(*fullsolves), zorder=15, s=42, marker='*', color=solvedcolor)
    if nosolves:
        scatter_outline(*zip(*nosolves), zorder=15, s=32, marker='X', color=unsolvedcolor)
...
class Graphs(commands.Cog):
    ...
    async def centile(self, ctx, *args: str):
        ...
        for user, point in users_to_mark.items():
            ...
            apos = ('left', 'top') if point[0] <= (xmax + xmin) // 2 else ('right',
                                                                           'bottom')
            plt.annotate(astr,
                         xy=point,
                         xytext=(0, 0),
    ...
    async def country(self, ctx, *countries):
        ...
        if not countries:
            ...
            data = [[user.country, user.rating] for _, user in users
    ...
    async def visualrank(self, ctx, contest_id: int, *args: str):
        ...
        if in_server:
            guild_handles = set(handle for discord_id, handle in
                                cf_common.user_db.get_handles_for_guild(ctx.guild.id))
            rating_changes = [
                rating_change for rating_change in rating_changes if
                rating_change.handle in guild_handles or rating_change.handle in handles
            ]
