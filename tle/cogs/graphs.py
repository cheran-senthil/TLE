...
def take_extreme (user, contest, submissions, problems):
    ...
    print (type (submissions[0].problem))
    ...
    for sub in by_user:
        print (sub.verdict, sub.problem.name, sub.problem.rating)
    ...
    print (contest.name, min_rating, max_rating)
...
class Graphs(commands.Cog):
    ...
    async def extreme(self, ctx, *args: str):
        """Plots Codeforces lowest unsolved problem/highesy solved on codeforces graph for the handle provided."""
        args = list(args)
        if '+zoom' in args:
            zoom = True
            args.remove('+zoom')
