...
def _get_embed_fields_from_contests(contests):
    infos = [(contest.name, str(contest.id),
    ...
    for name, id_str, start, duration, url in infos:
        value = _get_formatted_contest_desc(id_str, start, duration, url,
                                            max_duration_len)
...
class Contests(commands.Cog):
        ...
        for start_time, contests in self.start_time_map.items():
            for before_mins in before:
                ...
                task = asyncio.create_task(
                    _send_reminder_at(channel, role, contests, before_secs,
    ...
    async def future(self, ctx):
        ...
        await self._send_contest_list(ctx,
    ...
    async def here(self, ctx, role: discord.Role, *before: int):
        ...
        cf_common.user_db.set_reminder_settings(ctx.guild.id, ctx.channel.id, role.id,
                                                json.dumps(before))
        await ctx.send(
    ...
    async def on(self, ctx):
        ...
        if role in ctx.author.roles:
            ...
            await ctx.author.add_roles(role,
                                       reason='User subscribed to contest reminders')
    ...
    @staticmethod
    def _get_cf_or_ioi_standings_table(problem_indices,
                                       handle_standings,
                                       deltas=None,
                                       *,
                                       mode):
    ...
    def _make_standings_pages(self,
        ...
        delta_chunks = paginator.chunkify(
            deltas, _STANDINGS_PER_PAGE) if deltas else [None] * num_chunks
    ...
    def _make_contest_embed_for_ranklist(ranklist):
        ...
        if contest.phase == 'CODING':
            elapsed = cf_common.pretty_time_format(now - contest.startTimeSeconds,
                                                   shorten=True)
    ...
    @staticmethod
    def _make_contest_embed_for_vc_ranklist(ranklist,
                                            vc_start_time=None,
                                            vc_end_time=None):
        contest = ranklist.contest
        embed = discord_common.cf_color_embed(title=contest.name, url=contest.url)
        ...
        if vc_start_time and vc_end_time:
            ...
            remaining = cf_common.pretty_time_format(max(0, vc_end_time - now),
                                                     shorten=True)
            msg = f'{elapsed} elapsed{en}|{en}{remaining} remaining'
            embed.add_field(name='Tick tock', value=msg, inline=False)
        return embed
    ...
    async def ranklist(self, ctx, contest_id: int, *handles: str):
        ...
        await self._show_ranklist(channel=ctx.channel,
                                  contest_id=contest_id,
                                  handles=handles,
                                  ranklist=ranklist)

    async def _show_ranklist(self,
                             channel,
                             contest_id: int,
                             handles: [str],
                             ranklist,
        ...
        pages = self._make_standings_pages(contest, problem_indices, handle_standings,
                                           deltas)
        paginator.paginate(self.bot,
                           channel,
                           pages,
    ...
    async def ratedvc(self, ctx, contest_id: int, *members: discord.Member):
        ...
        if intersection:
            busy_members = ", ".join([
                ctx.guild.get_member(int(member_id)).mention for member_id in intersection
            ])
        ...
        cf_common.user_db.create_rated_vc(contest_id, start_time, finish_time,
                                          ctx.guild.id, [member.id for member in members])
    ...
    async def _watch_rated_vc(self, vc_id: int):
        ...
        async def has_running_subs(handle):
            return [
                ...
                if sub.verdict == 'TESTING' and sub.problem.contestId == vc.contest_id
        ...
        if running_subs_flag:
            ...
            await channel.send(embed=discord_common.embed_alert(msg),
                               delete_after=_WATCHING_RATED_VC_WAIT_TIME)
        if now < vc.finish_time or running_subs_flag:
            # Display current standings
            await channel.send(embed=self._make_contest_embed_for_vc_ranklist(
                ranklist, vc.start_time, vc.finish_time),
                               delete_after=_WATCHING_RATED_VC_WAIT_TIME)
            await self._show_ranklist(channel,
                                      vc.contest_id,
                                      handles,
                                      ranklist=ranklist,
                                      vc=True,
        ...
        for handle, member_id in zip(handles, member_ids):
            ...
            rating_change_by_handle[handle] = RatingChange(handle=handle,
        ...
        await channel.send(embed=self._make_vc_rating_changes_embed(
    ...
    async def vcratings(self, ctx):
        ...
        def make_page(chunk, page_num):
            ...
            for index, (member, handle, rating) in enumerate(chunk):
                ...
                t += table.Data(_PER_PAGE * page_num + index, f'{member.display_name}',
                                handle, rating_str)
    ...
    @commands.command(brief='Plot vc rating for a list of at most 5 users',
                      usage='@user1 @user2 ..')
