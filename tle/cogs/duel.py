...
class Dueling(commands.Cog):
    ...
    async def selfregister(self, ctx):
        ...
        if not cf_common.user_db.get_handle(ctx.author.id, ctx.guild.id):
            raise DuelCogError(f'{ctx.author.mention}, you cannot register yourself as a duelist without setting your handle.')
    ...
    async def complete(self, ctx):
        ...
        async def get_solve_time(userid):
            ...
            subs = [sub for sub in await cf.user.status(handle=handle) if (sub.verdict == 'OK' or sub.verdict == 'TESTING') and sub.problem.contestId == contest_id and sub.problem.index == index]
    ...
    async def vshistory(self, ctx, member1: discord.Member = None, member2: discord.Member = None):
        ...
        for _, _, _, _, challenger, challengee, winner in data:
            if winner != Winner.DRAW:
                ...
                draws += 1
        message = discord.utils.escape_mentions(f'`{member1.display_name}` ({wins}/{draws}/{losses}) `{member2.display_name}`')
    ...
    async def ranklist(self, ctx):
        ...
        users = [(member, cf_common.user_db.get_handle(member.id, ctx.guild.id), rating) for member, rating in users if member is not None and cf_common.user_db.get_num_duel_completed(member.id) > 0]
    ...
    async def rating(self, ctx, *members: discord.Member):
        ...
        labels = [gc.StrWrap('{} ({})'.format(ctx.guild.get_member(duelist).display_name, rating_data[-1][1])) for duelist, rating_data in plot_data.items()]
