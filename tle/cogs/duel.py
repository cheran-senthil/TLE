...
def complete_duel(duelid, guild_id, win_status, winner, loser, finish_time, score, dtype):
    ...
    rc = cf_common.user_db.complete_duel(duelid, win_status, finish_time, winner.id,
                                         loser.id, delta, dtype)
    if rc == 0:
...
class Dueling(commands.Cog):
    ...
    async def challenge(self, ctx, opponent: discord.Member, rating: int = None):
        ...
        solved = {
            sub.problem.name
            for subs in submissions for sub in subs if sub.verdict != 'COMPILATION_ERROR'
        }
        seen = {
            name
        ...
        def get_problems(rating):
            return [
                ...
                and prob.name not in seen and not any(
                    ...
                    for handle in handles) and not cf_common.is_nonstandard_problem(prob)
            ]
    ...
    async def accept(self, ctx):
        ...
        await ctx.send(f'Starting duel: {challenger.mention} vs {ctx.author.mention}',
                       embed=embed)
    ...
    async def complete(self, ctx):
        ...
        async def get_solve_time(userid):
            ...
            subs = [
                ...
                and sub.problem.contestId == contest_id and sub.problem.index == index
            ]
        ...
        if challenger_time and challengee_time:
            if challenger_time != challengee_time:
                ...
                embed = complete_duel(duelid, ctx.guild.id, win_status, winner, loser,
                                      min(challenger_time, challengee_time), 1, dtype)
                await ctx.send(
                    f'Both {challenger.mention} and {challengee.mention} solved it but {winner.mention} was {diff} faster!',
                    embed=embed)
            else:
                embed = complete_duel(duelid, ctx.guild.id, Winner.DRAW, challenger,
                                      challengee, challenger_time, 0.5, dtype)
                await ctx.send(
            ...
            embed = complete_duel(duelid, ctx.guild.id, Winner.CHALLENGER, challenger,
                                  challengee, challenger_time, 1, dtype)
            await ctx.send(f'{challenger.mention} beat {challengee.mention} in a duel!',
                           embed=embed)
        elif challengee_time:
            embed = complete_duel(duelid, ctx.guild.id, Winner.CHALLENGEE, challengee,
                                  challenger, challengee_time, 1, dtype)
    ...
    async def draw(self, ctx):
        ...
        if now - start_time < _DUEL_NO_DRAW_TIME:
            draw_time = cf_common.pretty_time_format(start_time + _DUEL_NO_DRAW_TIME -
                                                     now)
            await ctx.send(
        ...
        embed = complete_duel(duelid, ctx.guild.id, Winner.DRAW, offerer, ctx.author, now,
                              0.5, dtype)
        await ctx.send(f'{ctx.author.mention} accepted draw offer by {offerer.mention}.',
                       embed=embed)
    ...
    async def profile(self, ctx, member: discord.Member = None):
        ...
        if wins:
            ...
            embed.add_field(name='Fastest win',
                            value=duel_to_string(wins[0]),
                            inline=False)
            embed.add_field(name='Slowest win',
                            value=duel_to_string(wins[-1]),
                            inline=False)
    ...
    async def vshistory(self,
        ...
        pages = self._paginate_duels(
            data, f'{member1.display_name} ({w}/{d}/{l}) {member2.display_name}',
            ctx.guild.id, False)
    ...
    async def history(self, ctx, member: discord.Member = None):
        ...
        pages = self._paginate_duels(data, f'dueling history of {member.display_name}',
                                     ctx.guild.id, False)
        paginator.paginate(self.bot,
                           ctx.channel,
                           pages,
    ...
    async def ranklist(self, ctx):
        ...
        users = [(member, cf_common.user_db.get_handle(member.id, ctx.guild.id), rating)
                 for member, rating in users if member is not None
