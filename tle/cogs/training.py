    ...
    def doSolved(self, rating, duration):
        ...
        if self.mode != TrainingMode.NORMAL and self.mode != TrainingMode.SURVIVAL:
            if duration > self.timeleft:
                ...
                self.timeleft = int(
                    min(
                        self.timeleft - duration + self._getBaseTime(),
                        2 * self._getBaseTime(),
                    )
                )
        else:
...
class Training(commands.Cog):
    ...
    async def _postProblemFinished(
        self,
        ctx,
        handle,
        name,
        ...
        embed.add_field(
            name='Lives left:',
            value=gamestate.lives if gamestate.lives is not None else 'Inf',
        )
        await ctx.send(text, embed=embed)

    async def _postProblem(
        self,
        ctx,
        handle,
        problemName,
    ...
    async def _completeCurrentTrainingProblem(
        ...
        if rc == 1:
            await self._postProblemFinished(
                ctx,
                handle,
                name,
                contest_id,
