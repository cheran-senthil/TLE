...
class Codeforces(commands.Cog):
    ...
    async def teamrate(self, ctx, *args: str):
        ...
        if is_entire_server:
            ...
            ratings = [cf_user.maxRating if peak else cf_user.rating
                       for user_id, cf_user in res if cf_user.rating is not None]
            user_str = '+server'
        else:
            def normalize(x):
            ...
            ratings = [(user.maxRating if peak else user.rating,
