...
class Codeforces(commands.Cog):
    ...
    @commands.command(
        ...
        usage=
        '[handles] [+hardest] [+practice] [+contest] [+virtual] [+outof] [+team] [+tag..] [r>=rating] [r<=rating] [d>=[[dd]mm]yyyy] [d<[[dd]mm]yyyy] [c+marker..] [i+index..]'
    )
    ...
    async def mashup(self, ctx, *args):
        ...
        problems = [
            prob for prob in cf_common.cache2.problem_cache.problems
            if abs(prob.rating - rating) <= 100 and prob.name not in solved and not any(
    ...
    async def gitgud(self, ctx, delta: int = 0):
        ...
        problems = [
            ...
            if (prob.rating == rating +
                delta and prob.name not in solved and prob.name not in noguds)
        ]
    ...
    async def gotgud(self, ctx):
        ...
        rc = cf_common.user_db.complete_challenge(user_id, challenge_id, finish_time,
                                                  delta)
        if rc == 1:
    ...
    async def nogud(self, ctx):
        ...
        if finish_time - issue_time < _GITGUD_NO_SKIP_TIME:
            skip_time = cf_common.pretty_time_format(issue_time + _GITGUD_NO_SKIP_TIME -
                                                     finish_time)
    ...
    async def teamrate(self, ctx, *args: str):
        ...
        if is_entire_server:
            ...
            ratings = [(rating(user), 1) for user_id, user in res
            ...
            cf_handles = await cf_common.resolve_handles(ctx,
                                                         self.converter,
                                                         parsed_handles,
                                                         mincnt=1,
                                                         maxcnt=1000)
        ...
        embed = discord.Embed(title=user_str,
